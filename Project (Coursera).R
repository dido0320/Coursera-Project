...
path <- getwd()
d <- 'UCI HAR Dataset'
DT1<-read.table(read.table(file.path(path, d, 'train', 'subject_train.txt')))
DT2 <- data.table(read.table(file.path(path, d, 'test', 'subject_test.txt')))
DT3 <- rbind(DT1, DT2)
#read activities
DAT1<-DAT1<- data.table(read.table(file.path(path, d, 'train','Y_train.txt')))
DAT2<-data.table(read.table(file.path(path, d, 'train','Y_test.txt')))
...
datrain.set2 <- data.table(read.table(file.path(path,d,'test','X_test.txt')))
rbind(Train.set1,Train.set2)
# combine sebject/activity/set data
TEST<- cbind(SA,Trainset)
...
AT_Tidy<- AT %>% group_by(subject, activity_type)  %>% summarise_each(funs(mean)) %>% filter()
